...
public static class RawInputListenService
{
  ...
  private static void RegisterDevices()
  {
    ...
    try
    {
      // 重新将设备消息目标指向我们的 nativeWindow
      // 注意：当 Unity 抢走焦点后，再次调用这个方法会覆盖 Unity 的设置，
      // 重新将消息“吸”到我们的隐藏窗口来。
      ...
      Debug.Log("RawInput devices registered (Focus Restored)");
    }
    catch (Exception ex)
    {
      Debug.LogError("Failed to register RawInput devices: " + ex.Message);
    }
  }
  ...
  {
    // 只有当服务开启且应用程序重新获得焦点时才执行
    if (serviceEnabled && hasFocus)
    {
      // Unity 刚刚获得了焦点，它可能刚刚重置了 RawInput 设置。
      // 我们需要稍微延迟一下或者直接覆盖它。通常直接调用即可，
      // 但如果发现冲突，可以使用协程延迟一帧(需要 MonoBehaviour)。
      // 在静态类中，直接调用通常有效，因为 Unity 的内部处理通常在事件触发前已完成。
      RegisterDevices();
    }
  }
  ...
  static bool[] hidKeytates = new bool[0xFF+1];
